...
class act_toggle(client.Cog):
    ...
    async def act1(self,ctx):
        if ctx.guild.id in self.b.act2: # Switch to Act 1 if in Act 2
            self.b.act2.remove(ctx.guild.id)
    ...
    async def act2(self,ctx):
        if ctx.guild.id not in self.b.act2: # Switch to Act 2 if in Act 1
            self.b.act2.insert(0, ctx.guild.id)
